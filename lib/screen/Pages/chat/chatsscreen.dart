          ...
          children: [
            Expanded(
              child: ListView.separated(
                ...
                {
                  var m= socailCubit.get(context).masseage[index].senderuid;
                   if(socailCubit.get(context).model!.uId ==m)
                   {
                     return sendchat(socailCubit.get(context).masseage[index]);
                   }
                   else 
                   {
                     return reciverchat(socailCubit.get(context).masseage[index]);
                   }
                },
               itemCount: socailCubit.get(context).masseage.length,
            ...
            child: Row(
             ...
             MaterialButton(minWidth: 1,height: 45.0,color: Colors.blue,onPressed: (){
               socailCubit.get(context).sendmassages(text: masscont.text, datatime: DateTime.now().toString(), reciveruid: model.uId);
